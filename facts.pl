...
h6(Board,NewBoard,T):- another_player(T,T1),all_permutations(X,Y,'e'),all_permutations(X,Y,'e'),select_positions(X,Y),select_positions(Y,Z),X\=Z,Y\=Z,X\=Y,\+ all_permutations(X,Y,Z),available_position(Board,Y),position_has_Tpiece(Board,X,T1),
